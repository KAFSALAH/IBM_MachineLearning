...
# User profile model
    ...
    profile_sim_threshold = st.sidebar.slider('User Profile Similarity Threshold %',
                                              min_value=0, max_value=100,
                                              value=50, step=10)
    params['user_threshold'] = profile_sim_threshold
# Clustering model
    ...
    pca_no = st.sidebar.slider('Number of PCA components', min_value=0,
                                    max_value = 15, value = 5, step = 1)
    params['pca_no'] = pca_no
elif model_selection == backend.models[4]:
    max_k = st.sidebar.slider('number of K_max', min_value=1,max_value = 50,value = 10, step= 1)
    params['max_k'] = max_k
elif model_selection == backend.models[5]:
    nnf_epoch = st.sidebar.slider('number of epochs', min_value =1, max_value = 50, value = 10,step = 1)
    params['nnf_epoch'] = nnf_epoch
